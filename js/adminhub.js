/** SUPER ADMIN, BROADCAST, ADMIN TOOL, Coaching & Monitoring
 * js/adminhub.js
...
window.ensureAgentProfileModal = async function() {
    ...
    try {
        const html = await fetch('tabs/agentprofiles.html?v=7').then(r => r.ok ? r.text() : Promise.reject(new Error('Profile template unavailable')));
...
function getAdminPermissions(adminEmail) {
    ...
    // 0000 is COMPLETELY RESTRICTED
    const restricted = (window._RESTRICTED_ADMIN_IDS || ['0000']).map(s => String(s).toLowerCase());
    if (restricted.includes(email)) {
        return {
            isSuper: false, canSeeStats: false, canSeeAdminTools: false,
            canSeeTrivia: false, canSeeSuper: false, canSeeManagement: false
        };
    }
    
    // ROSE is SUPER ADMIN — sees everything including management tabs
    if (email === 'rose') {
        return {
            isSuper: true, canSeeStats: true, canSeeAdminTools: true,
            canSeeTrivia: true, canSeeSuper: true, canSeeManagement: true
        };
    }
    
    // MOMO and other admins — see stats/perf tabs only, NOT management
    if (email === 'momo') {
        return {
            isSuper: false, canSeeStats: true, canSeeAdminTools: false,
            canSeeTrivia: true, canSeeSuper: false, canSeeManagement: false
        };
    }
    
    // NADIA — stats + admin tools (file upload)
    if (email === 'nadia') {
        return {
            isSuper: false, canSeeStats: true, canSeeAdminTools: true,
            canSeeTrivia: true, canSeeSuper: false, canSeeManagement: false
        };
    }
...
// Get today's date in ISO format (Guyana timezone)
function getTodayISOGuyana() {
    const now = new Date();
    const guyanaTime = new Date(now.toLocaleString('en-US', { timeZone: 'America/Guyana' }));
    return guyanaTime.toISOString().split('T')[0];
}

let tbCurrentTeam = 'all';

// Load and render the Target Board
window.loadTargetBoard = async function() {
    const tbody = document.getElementById('tb-table-body');
    if (!tbody) return;
    
    tbody.innerHTML = '<tr><td colspan="9" class="p-8 text-center text-slate-500"><i class="fas fa-spinner fa-spin mr-2"></i> Loading...</td></tr>';
    
    const today = getTodayISOGuyana();
    
    // Update date display
    const dateEl = document.getElementById('tb-date');
    if (dateEl) {
        const guyanaDate = new Date();
        dateEl.textContent = guyanaDate.toLocaleDateString('en-US', { 
            weekday: 'short', 
            month: 'short', 
            day: 'numeric',
    ...
    try {
        // Get all goals for today from Firestore
        let goals = {};
        if (typeof db !== 'undefined' && db) {
            const goalsSnap = await db.collection('agent_goals').where('date', '==', today).get();
            goalsSnap.forEach(doc => {
                const data = doc.data();
                goals[data.agentId] = data;
            });
        }
        
        // Get today's stats from live dashboard state
        let todayStats = [];
        if (window._asLastLiveState && window._asLastLiveState.agents) {
            todayStats = window._asLastLiveState.agents;
        }
        
        // Get roster from window.allAgentProfiles
        const agents = window.allAgentProfiles || [];
        
        // Filter by team
        let filteredAgents = agents;
        if (tbCurrentTeam !== 'all') {
            filteredAgents = agents.filter(a => a.team === tbCurrentTeam);
        }
        
        if (filteredAgents.length === 0) {
            tbody.innerHTML = '<tr><td colspan="9" class="p-8 text-center text-slate-500">No agents found</td></tr>';
            return;
        }
        ...
        filteredAgents.forEach(agent => {
            const goal = goals[agent.userId || agent.ytelId];
            if (goal) {
                goalsSet++;
                const stat = todayStats.find(s => s.ytelId === (agent.userId || agent.ytelId) || s.name === agent.name);
                const actual = stat ? (stat.dailyLeads || 0) : 0;
                if (actual >= goal.high || goal.hitHigh) hitHigh++;
                else if (actual >= goal.low || goal.hitLow) hitLow++;
            }
        });
        
        document.getElementById('tb-goal-set').textContent = goalsSet;
        document.getElementById('tb-hit-low').textContent = hitLow;
        document.getElementById('tb-hit-high').textContent = hitHigh;
        document.getElementById('tb-no-goal').textContent = filteredAgents.length - goalsSet;
        ...
        filteredAgents.forEach((agent, idx) => {
            ...
            const teamClass = agent.team === 'BB' ? 'bg-blue-500/20 text-blue-400' : 
                             (agent.team === 'RM' ? 'bg-cyan-500/20 text-cyan-400' : 'bg-purple-500/20 text-purple-400');
            
            if (!goal) {
                rows += `
                    <tr class="border-b border-white/5">
                        <td class="p-3 text-slate-500">${idx + 1}</td>
                        <td class="p-3 font-bold text-white">${escapeHtml(agent.fullName || agent.name)}</td>
                        <td class="p-3"><span class="px-2 py-1 rounded text-[9px] font-black ${teamClass}">${agent.team || 'PR'}</span></td>
                        <td class="p-3 text-center text-slate-500">—</td>
                        <td class="p-3 text-center text-slate-500">—</td>
                        <td class="p-3 text-center text-cyan-400 font-bold">${actual}</td>
                        <td class="p-3 text-center">—</td>
                        <td class="p-3 text-center">—</td>
                        <td class="p-3 text-center"><span class="px-2 py-1 rounded text-[9px] font-black bg-slate-500/20 text-slate-400">No Goal</span></td>
                    </tr>
                `;
                return;
            }
            
            const lowPct = Math.min(100, (actual / goal.low) * 100);
            const highPct = Math.min(100, (actual / goal.high) * 100);
            let statusClass = '', statusText = '';
            
            if (actual >= goal.high || goal.hitHigh) {
                statusClass = 'bg-cyan-500/20 text-cyan-400';
                statusText = '🔥 High Hit!';
            } else if (actual >= goal.low || goal.hitLow) {
                statusClass = 'bg-green-500/20 text-green-400';
                statusText = '✅ Low Hit';
            } else {
                statusClass = 'bg-yellow-500/20 text-yellow-400';
                statusText = `${goal.low - actual} to low`;
            }
            ...
            rows += `
                <tr class="border-b border-white/5 hover:bg-white/5 transition">
                    ...
                    <td class="p-3 font-bold text-white">${escapeHtml(agent.fullName || agent.name)}</td>
                    <td class="p-3"><span class="px-2 py-1 rounded text-[9px] font-black ${teamClass}">${agent.team || 'PR'}</span></td>
                    <td class="p-3 text-center font-bold text-green-400">${goal.low}</td>
                    <td class="p-3 text-center font-bold text-cyan-400">${goal.high}</td>
                    <td class="p-3 text-center text-cyan-400 font-bold text-lg">${actual}</td>
                    <td class="p-3 text-center">
                        <div class="w-24 bg-black/40 rounded-full h-2 overflow-hidden inline-block">
                            <div class="h-full rounded-full bg-green-500" style="width: ${lowPct}%"></div>
                        </div>
                        <span class="text-[9px] text-slate-500 ml-1">${Math.round(lowPct)}%</span>
                    </td>
                    <td class="p-3 text-center">
                        <div class="w-24 bg-black/40 rounded-full h-2 overflow-hidden inline-block">
                            <div class="h-full rounded-full bg-cyan-500" style="width: ${highPct}%"></div>
                        </div>
                        <span class="text-[9px] text-slate-500 ml-1">${Math.round(highPct)}%</span>
                    </td>
                    <td class="p-3 text-center"><span class="px-2 py-1 rounded text-[9px] font-black ${statusClass}">${statusText}</span></td>
                </tr>
            `;
        });
        ...
        console.error('Target board error:', error);
        tbody.innerHTML = '<tr><td colspan="9" class="p-8 text-center text-red-400">Failed to load target board</td></tr>';
    }
};

// Filter target board by team
window.tbFilterTeam = function(team) {
    tbCurrentTeam = team;
    
    // Update active state on buttons
    const buttons = ['all', 'BB', 'PR', 'RM'];
    buttons.forEach(t => {
        const btn = document.getElementById(`tb-filter-${t.toLowerCase()}`);
        if (btn) {
            if (t === team) {
                btn.classList.add('active', 'bg-cyan-500/20');
                btn.classList.remove('bg-white/5');
                btn.style.color = '#22d3ee';
                btn.style.borderColor = '#06b6d4';
            } else {
                btn.classList.remove('active', 'bg-cyan-500/20');
...
window.ahInitOverview = function() {
    ...
    if (typeof window.listenForAgentReports === 'function') {
        window.listenForAgentReports(reports => {
            ...
            // Refresh target board if tab is active
            if (ahCurrentSubTab === 'targetboard') {
                loadTargetBoard();
            }
        });
    }
...
window.switchAttView = function(view) {
    ...
    const vEl = document.getElementById(`att-view-${view}`);
    ...
    ['daily', 'weekly', 'monthly'].forEach(v => {
        ...
        if (b) {
            b.className = (v === view)
                ? "px-4 py-2 rounded-xl text-[10px] font-black bg-blue-500 text-white shadow-lg shadow-blue-900/40"
                : "px-4 py-2 rounded-xl text-[10px] font-black text-slate-400 hover:text-white transition";
        }
    });
...
window.filterAttTeam = function(team) {
    ...
    if (ahAttCurrentView === 'monthly') ahLoadMonthlyMatrix();
};
...
async function renderDailyAttendance() {
    ...
    if (typeof window.getAttendanceForDate === 'function') {
        rtdbRecords = await window.getAttendanceForDate(ahAttSelectedDate);
    }

    // Merge roster with RTDB records
    const sourceData = roster
        .filter(p => p.status !== 'Inactive')
        .map(p => {
            const rec = rtdbRecords[String(p.userId)];
            return {
                name: p.fullName,
                ytelId: p.userId,
                team: p.team || '',
    ...
    const filtered = sourceData.filter(a => {
        ...
        return normalizeTeam(a.team, a.rawName || a.name) === ahAttFilterTeam;
    });

    // Update summary counters
    const _allStatuses = ['Training','Present','Late','Absent','Personal Out','Vacation','Sick','Off','Holiday'];
    const _sumColors = { Training:'text-indigo-400', Present:'text-green-400', Late:'text-yellow-400', Absent:'text-red-400', 'Personal Out':'text-orange-400', Vacation:'text-purple-400', Sick:'text-rose-400', Off:'text-slate-400', Holiday:'text-cyan-400' };
    const summaryEl = document.getElementById('att-summary-counts');
    if (summaryEl) {
        const parts = _allStatuses
            .map(s => { const n = filtered.filter(a => a.status === s).length; return n > 0 ? `<span class="${_sumColors[s]} font-black text-[11px]">${n} ${s}</span>` : ''; })
            .filter(Boolean).join('');
        summaryEl.innerHTML = parts + `<span class="text-slate-400 font-black text-[11px]">${filtered.length} Total</span>`;
    }

    if (filtered.length === 0) {
        list.innerHTML = '<tr><td colspan="6" class="py-10 text-center text-slate-500 font-bold uppercase tracking-widest">No agents found for this filter.</td></tr>';
        return;
    }
    ...
    list.innerHTML = filtered.map((a, i) => {
        ...
        const _sc = { Training:'text-indigo-400', Present:'text-green-400', Late:'text-yellow-400', Absent:'text-red-400', 'Personal Out':'text-orange-400', Vacation:'text-purple-400', Sick:'text-rose-400', Off:'text-slate-400', Holiday:'text-cyan-400' };
        const _dc = { Training:'bg-indigo-500', Present:'bg-green-500', Late:'bg-yellow-500', Absent:'bg-red-500/60', 'Personal Out':'bg-orange-500', Vacation:'bg-purple-500', Sick:'bg-rose-500', Off:'bg-slate-500', Holiday:'bg-cyan-500' };
        const statusClass = _sc[a.status] || 'text-slate-400';
        const dotClass = _dc[a.status] || 'bg-slate-500';
        const nameEsc = encodeURIComponent(a.name);
        const teamEsc = encodeURIComponent(team);
        const idSafe = String(a.ytelId).replace(/[^a-zA-Z0-9_-]/g,'_');
        const notesEsc = encodeURIComponent(a.notes || '');
        return `<tr class="hover:bg-white/5 transition att-row" data-agent-id="${a.ytelId}" data-agent-name="${nameEsc}" data-agent-team="${teamEsc}" data-agent-status="${a.status}" data-agent-time="${a.loginTime}" data-agent-notes="${notesEsc}">
            <td class="py-3 px-2">
                <input type="checkbox" class="att-row-cb w-3.5 h-3.5 accent-blue-500 cursor-pointer" data-id="${a.ytelId}" onchange="ahUpdateBulkToolbar()">
            </td>
            ...
            <td class="py-3">
                <span class="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest ${statusClass}">
                    <span class="w-1.5 h-1.5 rounded-full ${dotClass}"></span>
                    ${a.status}
                </span>
            </td>
            <td class="py-3 text-right tabular-nums text-[10px] text-slate-400 font-bold">${a.loginTime}</td>
            <td class="py-3 text-center">
                <button onclick="ahOpenAttEdit('${a.ytelId}','${nameEsc}','${teamEsc}','${a.status}','${a.loginTime}','${ahAttSelectedDate}','${notesEsc}')"
                    class="w-7 h-7 rounded-lg bg-yellow-500/10 hover:bg-yellow-500/20 border border-yellow-500/30 text-yellow-400 text-[10px] transition" title="Edit">
...
window.ahUpdateBulkToolbar = function() {
    const cbs = document.querySelectorAll('.att-row-cb:checked');
    const toolbar = document.getElementById('att-bulk-toolbar');
    const countEl = document.getElementById('att-bulk-count');
    const selectAll = document.getElementById('att-select-all');
    const total = document.querySelectorAll('.att-row-cb').length;
    if (!toolbar) return;
    toolbar.style.display = cbs.length > 0 ? 'flex' : 'none';
    if (countEl) countEl.textContent = cbs.length + ' Selected';
    if (selectAll) selectAll.indeterminate = cbs.length > 0 && cbs.length < total;
    if (selectAll && cbs.length === total && total > 0) selectAll.checked = true;
    if (selectAll && cbs.length === 0) selectAll.checked = false;
};

window.ahToggleSelectAll = function(checked) {
    document.querySelectorAll('.att-row-cb').forEach(cb => cb.checked = checked);
    window.ahUpdateBulkToolbar();
};

window.ahBulkClearSelection = function() {
    document.querySelectorAll('.att-row-cb').forEach(cb => cb.checked = false);
    const sa = document.getElementById('att-select-all');
    if (sa) { sa.checked = false; sa.indeterminate = false; }
    const sel = document.getElementById('att-bulk-status-sel');
    if (sel) sel.value = '';
    window.ahUpdateBulkToolbar();
};

window.ahApplyBulkStatus = function() {
    const sel = document.getElementById('att-bulk-status-sel');
    if (!sel || !sel.value) { alert('Please choose a status first.'); return; }
    window.ahBulkMark(sel.value);
};

window.ahBulkMark = async function(status) {
    const cbs = document.querySelectorAll('.att-row-cb:checked');
    if (cbs.length === 0) return;

    const toolbar = document.getElementById('att-bulk-toolbar');
    const countEl = document.getElementById('att-bulk-count');
    if (countEl) countEl.textContent = 'Saving ' + cbs.length + ' agents...';

    const date = ahAttSelectedDate;
    const saves = [];

    cbs.forEach(cb => {
        const row = cb.closest('tr.att-row');
        if (!row) return;
        const agentId = row.dataset.agentId;
        const name = decodeURIComponent(row.dataset.agentName || '');
        const team = decodeURIComponent(row.dataset.agentTeam || '');
        const existingTime = row.dataset.agentTime;
        const record = {
    ...
    try {
        ...
        window.ahBulkClearSelection();
        await renderDailyAttendance();
    } catch(e) {
        console.error('Bulk attendance save error:', e);
        if (countEl) countEl.textContent = 'Error saving — check console';
    }
};

// ===== ATTENDANCE EDIT FUNCTIONS =====

window.ahOpenAttEdit = function(agentId, encodedName, encodedTeam, status, time, date, encodedNotes) {
    const name = decodeURIComponent(encodedName);
    document.getElementById('att-edit-agent-id').value = agentId;
    document.getElementById('att-edit-agent-name').value = name;
    document.getElementById('att-edit-agent-team').value = decodeURIComponent(encodedTeam);
    document.getElementById('att-edit-date').value = date;
    document.getElementById('att-edit-name').textContent = name;
    const statusSel = document.getElementById('att-edit-status');
    if (statusSel) statusSel.value = status || 'Present';
    const timeInp = document.getElementById('att-edit-time');
    if (timeInp) {
        const t = (time && time !== '--:--') ? time : '';
        timeInp.value = t.length === 5 ? t : '';
    }
    const notesInp = document.getElementById('att-edit-notes');
    if (notesInp) notesInp.value = encodedNotes ? decodeURIComponent(encodedNotes) : '';
    const msg = document.getElementById('att-edit-status-msg');
    if (msg) msg.textContent = '';
    const panel = document.getElementById('att-edit-panel');
    if (panel) panel.style.display = 'flex';
    document.body.style.overflow = 'hidden';
};

window.ahSaveAttEdit = async function() {
    const agentId = document.getElementById('att-edit-agent-id').value;
    const agentName = document.getElementById('att-edit-agent-name').value;
    const agentTeam = document.getElementById('att-edit-agent-team').value;
    ...
    const notesEl = document.getElementById('att-edit-notes');
    const notes = notesEl ? notesEl.value.trim() : '';
    const msg = document.getElementById('att-edit-status-msg');

    if (!agentId || !date) { if (msg) { msg.textContent = 'Missing agent or date.'; msg.className = 'text-[11px] text-center text-red-400'; } return; }

    if (msg) { msg.textContent = 'Saving...'; msg.className = 'text-[11px] text-center text-blue-400'; }
    ...
    try {
        ...
        if (typeof window.rtdbSet === 'function' && typeof window.rtdbRef === 'function') {
            await window.rtdbSet(window.rtdbRef(`attendance/${date}/${agentId}`), record);
        }
        if (msg) { msg.textContent = '✓ Saved successfully'; msg.className = 'text-[11px] text-center text-green-400 font-black'; }
        setTimeout(() => {
            ahCloseAttEdit();
            renderDailyAttendance();
        }, 800);
    } catch(e) {
        console.error('Att edit save error:', e);
        if (msg) { msg.textContent = 'Error saving. Check console.'; msg.className = 'text-[11px] text-center text-red-400'; }
    }
};

window.ahCloseAttEdit = function() {
    const panel = document.getElementById('att-edit-panel');
...
window.ahLoadMonthlyMatrix = async function() {
    ...
    if (roster.length === 0) {
        container.innerHTML = '<div class="py-10 text-slate-500 text-center font-bold">No agents found for this filter.</div>';
        return;
    }
...
function handleLiveStateUpdate(state) {
    ...
    // Refresh target board if it's the active tab
    if (ahCurrentSubTab === 'targetboard') {
        loadTargetBoard();
    }
}
...
// ===== USER MANAGEMENT (sa-sect-usermgmt) =====
...
window.superLoadAdmins = function() {
    ...
    container.innerHTML = allAdmins.map(a => `
        <div class="flex items-center justify-between py-3 px-4 bg-white/5 rounded-xl border border-white/10">
            <div>
                <div class="text-white font-black text-[12px]">${a.name || a.email}</div>
                <div class="text-slate-500 text-[9px] font-bold uppercase tracking-widest">${a.email} · <span class="${a.role === 'super_admin' ? 'text-yellow-400' : 'text-cyan-400'}">${(a.role || 'admin').replace('_', ' ')}</span></div>
            </div>
            <button onclick="superRemoveAdminUser('${a.email}')" class="text-[9px] font-black text-red-400 hover:text-red-300 bg-red-500/10 hover:bg-red-500/20 border border-red-500/20 px-3 py-1.5 rounded-lg transition">Remove</button>
        </div>
    `).join('');
};
...
window.superAddNewAdmin = async function() {
    ...
    if (typeof window.addNewAdmin === 'function') {
        ...
        if (res.success) {
            ...
            document.getElementById('super-new-admin-password').value = '';
            superLoadAdmins();
...
window.ahSyncRosterFromSheet = async function() {
    // Now reads from Firebase RTDB (no Google Sheets)
    if (!window.rtdbRef || !window.rtdbGet) {
        console.warn('[AdminHub] RTDB helpers not ready');
        return;
    }
    try {
        const snap = await window.rtdbGet(window.rtdbRef('biz_master_roster'));
        let roster = snap.val() || [];
        if (!Array.isArray(roster)) roster = Object.values(roster);
        roster = roster.filter(a => a && a.fullName && a.userId && a.status !== 'Inactive');

        if (roster.length > 0) {
            console.log('[AdminHub] Roster synced from RTDB:', roster.length, 'agents');
            ...
            if (window.ahCurrentSubTab === 'attendance') renderDailyAttendance();
            if (window.ahCurrentSubTab === 'zero' && ahZeroPerfInitialized) ahInitZeroPerfLazy();
            if (window.ahCurrentSubTab === 'performance') {
                buildAvailableWeeks();
                populateWeekDropdown();
                loadWeeklyDataForWeek(ahSelectedWeek);
            }
            if (window.ahCurrentSubTab === 'targetboard') loadTargetBoard();
        }
    } catch (e) {
        console.error('[AdminHub] RTDB roster sync failed:', e);
    }
};
