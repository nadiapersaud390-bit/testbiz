...
(() => {
    ...
    function goalSettingsPath() {
        return `agent_goal_settings/${STATE.agentKey}`;
    }

    function dateISOInGuyana(value) {
        if (value === undefined || value === null) return '';
        const raw = String(value).trim();
        if (!raw) return '';

        // Calendar-only labels represent a Guyana day, not a UTC timestamp.
        const isoOnly = raw.match(/^(\d{4})-(\d{2})-(\d{2})$/);
        if (isoOnly) return `${isoOnly[1]}-${isoOnly[2]}-${isoOnly[3]}`;

        const namedDate = raw.match(/^(?:(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)(?:day)?,?\s+)?(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)\s+(\d{1,2}),\s*(\d{4})$/i);
        if (namedDate) {
            const monthMap = { jan: '01', feb: '02', mar: '03', apr: '04', may: '05', jun: '06', jul: '07', aug: '08', sep: '09', oct: '10', nov: '11', dec: '12' };
            const month = monthMap[namedDate[1].slice(0, 3).toLowerCase()];
            return `${namedDate[3]}-${month}-${String(namedDate[2]).padStart(2, '0')}`;
        }

        const numericDate = raw.match(/^(\d{1,2})[\/-](\d{1,2})[\/-](\d{4})$/);
        if (numericDate) {
            return `${numericDate[3]}-${String(numericDate[1]).padStart(2, '0')}-${String(numericDate[2]).padStart(2, '0')}`;
        }
    ...
    function isLiveStateForDate(state, targetDate) {
        if (!state || typeof state !== 'object') return false;

        // Prefer the report's explicit date. A report uploaded today for an older
        // day must not carry yesterday's transfers into today's goal progress.
        const explicitDates = [state.dateISO, state.reportDate, state.date, state.dateLabel]
            .filter(value => value !== undefined && value !== null && String(value).trim() !== '');
        for (const value of explicitDates) {
            const normalized = /^\d{4}-\d{2}-\d{2}$/.test(String(value).trim())
                ? String(value).trim()
                : dateISOInGuyana(value);
            if (normalized) return normalized === targetDate;
        }

        return state.pushedAt ? dateISOInGuyana(state.pushedAt) === targetDate : false;
    }

    function buildDailyGoalRecord(source, existing = null) {
        const normalized = normalizeGoals(source);
        if (!normalized) return null;
        const now = Date.now();
        return {
            agentId: STATE.agentId || normalized.agentId || '',
            agentName: STATE.profile?.name || STATE.profile?.fullName || normalized.agentName || '',
            ytelName: STATE.profile?.ytelName || normalized.ytelName || '',
            team: STATE.profile?.team || normalized.team || 'PR',
            date: STATE.date,
            low: normalized.low,
            high: normalized.high,
            hitLow: Boolean(existing?.hitLow),
            hitHigh: Boolean(existing?.hitHigh),
            setByAdmin: Boolean(source.setByAdmin),
            setByAgent: Boolean(source.setByAgent),
            lastSetBy: source.lastSetBy || (source.setByAdmin ? 'admin' : 'agent'),
            cadence: 'daily',
            progressResetsDaily: true,
            createdAt: existing?.createdAt || now,
            updatedAt: now
        };
    }

    function buildDailyGoalSettings(source) {
        const normalized = normalizeGoals(source);
        if (!normalized) return null;
        return {
            agentId: STATE.agentId || normalized.agentId || '',
    ...
    async function ensureDailyGoalRecord() {
        if (!firebaseReady() || !ensureStateFromSession()) return null;

        const [daySnapshot, settingsSnapshot] = await Promise.all([
            window.rtdbGet(window.rtdbRef(goalPath())),
            window.rtdbGet(window.rtdbRef(goalSettingsPath()))
        ]);

        const dayGoal = normalizeGoals(daySnapshot?.val?.());
        const settingsRaw = settingsSnapshot?.val?.();
        const settingsGoal = normalizeGoals(settingsRaw);

        // Migrate an existing dated goal into the new recurring daily settings.
        if (dayGoal && !settingsGoal) {
            const migratedSettings = buildDailyGoalSettings(dayGoal);
            if (migratedSettings) {
                await window.rtdbSet(window.rtdbRef(goalSettingsPath()), migratedSettings);
            }
            return dayGoal;
        }

        if (!settingsGoal) return dayGoal;

        const valuesChanged = !dayGoal || dayGoal.low !== settingsGoal.low || dayGoal.high !== settingsGoal.high ||
            Boolean(dayGoal.setByAdmin) !== Boolean(settingsRaw?.setByAdmin);
        if (!valuesChanged) return dayGoal;

        const dailyRecord = buildDailyGoalRecord(settingsRaw, dayGoal);
        if (!dailyRecord) return dayGoal;
        await window.rtdbSet(window.rtdbRef(goalPath()), dailyRecord);
        return dailyRecord;
    }
    ...
    function setupGoalLifecycleListeners() {
        ...
        const checkDailyRollover = () => {
            ...
            const currentDate = getGuyanaDateISO();
        ...
        clearInterval(STATE.rolloverTimer);
        STATE.rolloverTimer = setInterval(checkDailyRollover, 30000);
    ...
    function connectGoalRecord(attempt = 0) {
        ...
        // Create today's fresh record from the recurring daily target before
        // subscribing. Each date has independent hit flags and starts at zero.
        ensureDailyGoalRecord()
            .catch(error => console.warn('[Goals] Could not prepare daily goal record:', error))
            .finally(attachListener);
    }
        ...
        modal.innerHTML = `
            <div class="biz-goal-dialog" role="document">
                ...
                <div class="biz-goal-dialog-body">
                    ...
                    <p class="biz-goal-intro">Set or confirm your own daily low and high targets. Any existing target is prefilled and you may edit it.</p>
    ...
    async function saveGoalPopup() {
        ...
        try {
            ...
            const settings = buildDailyGoalSettings(payload);
            await Promise.all([
                window.rtdbSet(window.rtdbRef(goalPath()), payload),
                settings ? window.rtdbSet(window.rtdbRef(goalSettingsPath()), settings) : Promise.resolve()
            ]);
            clearPendingSync();
            closeGoalPopup();
    ...
    async function flushPendingGoalSync() {
        ...
        if (!pending) return;
        try {
            const settings = buildDailyGoalSettings(pending);
            await Promise.all([
                window.rtdbSet(window.rtdbRef(goalPath()), pending),
                settings ? window.rtdbSet(window.rtdbRef(goalSettingsPath()), settings) : Promise.resolve()
            ]);
            clearPendingSync();
        } catch (error) {
    ...
    function refreshActualTransfers() {
        ...
        let agents = [];
        const liveState = window._asLastLiveState;
        if (liveState && Array.isArray(liveState.agents)) {
            if (!isLiveStateForDate(liveState, STATE.date)) {
                STATE.actual = 0;
                return STATE.actual;
            }
            agents = liveState.agents;
        } else if (Array.isArray(window.allAgents)) {
            // allAgents is a fallback only. Reject it when its date marker is stale.
            const dateMarker = window.allAgents[0]?.todayName;
            if (dateMarker && dateISOInGuyana(dateMarker) !== STATE.date) {
                STATE.actual = 0;
                return STATE.actual;
            }
    ...
    // Admin helper: set a recurring daily goal for a specific agent.
    async function adminSetAgentGoals(agentId, low, high) {
        ...
        try {
            await Promise.all([
                window.rtdbSet(window.rtdbRef(`agent_goal_settings/${key}`), settings),
                window.rtdbSet(window.rtdbRef(`agent_goals/${key}/${date}`), todayRecord)
            ]);
            return { success: true, cadence: 'daily' };
        } catch (error) {
    ...
    async function adminGetAgentGoals(agentId) {
        ...
        const key = safeFirebaseKey(agentId);
        try {
            const [settingsSnapshot, todaySnapshot] = await Promise.all([
                window.rtdbGet(window.rtdbRef(`agent_goal_settings/${key}`)),
                window.rtdbGet(window.rtdbRef(`agent_goals/${key}/${date}`))
            ]);
            const settings = normalizeGoals(settingsSnapshot.val());
            if (settings) return { ...settings, cadence: 'daily', progressResetsDaily: true };

            const today = normalizeGoals(todaySnapshot.val());
            if (today) {
                // Legacy migration: make an existing target repeat every day.
                await window.rtdbSet(window.rtdbRef(`agent_goal_settings/${key}`), {
                    agentId: String(agentId || ''),
                    low: today.low,
                    high: today.high,
                    setByAdmin: Boolean(today.setByAdmin),
                    cadence: 'daily',
                    active: true,
                    progressResetsDaily: true,
                    updatedAt: Date.now()
                });
                return { ...today, cadence: 'daily', progressResetsDaily: true };
            }
            return null;
        } catch (_) {
            return null;
