...
async function loadFullRoster(force) {
    ...
    // Roster is loaded from Firebase RTDB via listenForMasterRoster in firebase.js
...
function render() {
    ...
    if (currentTab === 'chatroom') { if (crView) crView.classList.remove('hidden'); return; }
...
function switchTab(tab) {
    ...
    document.dispatchEvent(new CustomEvent('biz:tabchange', { detail: { tab } }));
    ...
    if (tab === 'chatroom' && typeof window.initChatroom === 'function') window.initChatroom();
...
function requestWeekly() {
    ...
    currentTab = 'weekly';
    window.currentTab = 'weekly';
    document.dispatchEvent(new CustomEvent('biz:tabchange', { detail: { tab: 'weekly' } }));
    updateTabUI();
    render();
    renderDaySubTabs();
}
