...
(function() {
    ...
    // Shared emoji library for admin + agent chatrooms.
    // Keep it Unicode-only so it works without an external emoji service.
    const EMOJI_GROUPS = [
    ...
    // The dashboard stores currentTab as a top-level `let`, so it is not always
    // available as window.currentTab. Resolve both sources and verify the view.
    function _isChatroomActive() {
        let activeTab = window.currentTab;
        if (!activeTab) {
            try {
                if (typeof currentTab !== 'undefined') activeTab = currentTab;
            } catch (e) {}
        }
        if (activeTab === 'chatroom') return true;

        const view = document.getElementById('chatroom-view');
        if (!view || view.classList.contains('hidden')) return false;
        try {
            return window.getComputedStyle(view).display !== 'none';
        } catch (e) {
            return true;
        }
    }

    function _syncFloatingChatVisibility() {
        const bubble = document.getElementById('cr-floating-bubble');
        const widget = document.getElementById('cr-floating-widget');
        const chatroomOpen = _isChatroomActive();
        ...
        // Leaving the full chatroom restores only the bubble. The side panel
        // remains closed until the user clicks the bubble or receives a message.
        if (bubble) {
    ...
    window._crSyncFloatingChatVisibility = _syncFloatingChatVisibility;
    document.addEventListener('biz:tabchange', function() {
        setTimeout(_syncFloatingChatVisibility, 0);
    });
        ...
        _fbFunctions.onValue(msgRef, async (snapshot) => {
            ...
            if (ch) {
                ...
                if (!initialLoad && messages.length > 0) {
                    ...
                    if (newest && newest.from !== me.id) {
                        _playNotificationSound();
                        if (!_isFirstLoad && !_isChatroomActive()) {
    ...
    // ═══════════════════════════════════════════
    function _buildEmojiPickerMarkup(withClose) {
        var html = '<div class="cr-emoji-picker-top">';
        html += '<div><div class="cr-emoji-picker-title">😊 Emojis</div><div class="cr-emoji-picker-subtitle">Tap an emoji to add it to your message</div></div>';
        if (withClose) html += '<button class="cr-emoji-close-btn" aria-label="Close emoji picker">×</button>';
        html += '</div>';
        EMOJI_GROUPS.forEach(function(group) {
            html += '<div class="cr-emoji-category"><span>' + group.icon + '</span>' + group.label + '</div>';
            group.items.forEach(function(emoji) {
                html += '<button type="button" class="cr-emoji-option" data-emoji="' + emoji + '" aria-label="Add emoji">' + emoji + '</button>';
            });
        });
        return html;
    }
    ...
    function _buildEmojiPickerWithClose(pickerId) {
        return _buildEmojiPickerMarkup(true);
    }

    function _buildEmojiPicker() {
        return _buildEmojiPickerMarkup(false);
    }
    ...
    window._crOpenFloatChat = function(channelId) {
        if (_isChatroomActive()) { _syncFloatingChatVisibility(); return; }
