(function () {
  ...
  style.id = 'simulator-text-mode-style';
  style.textContent = `
    ...
    */
        ...
        if (current[quality]) {
          options.push({
            ...
            customer_reply:
              ...
              '',
            next_step: current['next_' + quality] || ''
          });
        }
      });
    ...
    lines.forEach(function (line) {
      ...
      if ((match = line.match(/^Step\s*(\d+)?\s*:?[\s]*(.*)$/i))) {
        ...
        var stepNumber = cleanValue(match[1]);
        var stepTitle = cleanValue(match[2]);

        current = {
          label: stepTitle
            ? ((stepNumber ? 'Step ' + stepNumber + ' — ' : '') + stepTitle)
            : (stepNumber ? 'Step ' + stepNumber : ('Step ' + (steps.length + 1)))
        };
      ...
      else if ((match = line.match(/^(?:Next step|If) Best(?: go to)?:\s*(.+)$/i))) { current.next_best = cleanValue(match[1]); }
      else if ((match = line.match(/^(?:Next step|If) OK(?: go to)?:\s*(.+)$/i))) { current.next_ok = cleanValue(match[1]); }
      else if ((match = line.match(/^(?:Next step|If) Weak(?: go to)?:\s*(.+)$/i))) { current.next_weak = cleanValue(match[1]); }
      else if ((match = line.match(/^(?:Next step|If) Wrong(?: go to)?:\s*(.+)$/i))) { current.next_wrong = cleanValue(match[1]); }
    });
  ...
  function applyTextMode() {
    ...
    */
    if (typeof window._parseScriptContent !== 'function') {
      window._parseScriptContent = function (
        ...
        requiredOverride,
        forbiddenOverride,
        errorOptionsOverride,
        correctErrorsOverride
      ) {
        var parsed = parseStructuredScript(raw);
        function listValue(value) {
          var values = Array.isArray(value) ? value : String(value || '').split(/\r?\n|\s*;\s*/g);
          return values.map(function(item) { return cleanValue(String(item || '').replace(/^[-*•]+\s*/, '')); }).filter(Boolean);
        }
        var root = {};
        try {
          if (/^[\[{]/.test(String(raw || '').trim())) root = JSON.parse(String(raw || '').trim()) || {};
        } catch (_) {}
        var requiredPhrases = listValue(requiredOverride).length ? listValue(requiredOverride) : listValue(root.required_phrases || root.requiredPhrases || []);
        var forbiddenPhrases = listValue(forbiddenOverride).length ? listValue(forbiddenOverride) : listValue(root.forbidden_phrases || root.forbiddenPhrases || []);
        var callErrorOptions = listValue(errorOptionsOverride).length ? listValue(errorOptionsOverride) : listValue(root.call_error_options || root.callErrorOptions || []);
        var correctCallErrors = listValue(correctErrorsOverride).length ? listValue(correctErrorsOverride) : listValue(root.correct_call_errors || root.correctCallErrors || []);
        correctCallErrors.forEach(function(item) {
          if (!callErrorOptions.some(function(option) { return option.toLowerCase() === item.toLowerCase(); })) callErrorOptions.push(item);
        });
        ...
        return {
          ...
          call_type: cleanValue(callTypeOverride) || cleanValue(root.call_type || root.callType || '') || 'Training Call',
          correct_end_reason: cleanValue(endReasonOverride) || cleanValue(root.correct_end_reason || root.correctEndReason || ''),
          end_call_note: cleanValue(endNoteOverride) || cleanValue(root.end_call_note || root.endCallNote || ''),
          training_note: cleanValue(root.training_note || root.trainingNote || root.training_tip || root.trainingTip || root.error_found_note || root.errorFoundNote || ''),
          trainingNote: cleanValue(root.training_note || root.trainingNote || root.training_tip || root.trainingTip || root.error_found_note || root.errorFoundNote || ''),
          required_phrases: requiredPhrases,
          requiredPhrases: requiredPhrases,
          forbidden_phrases: forbiddenPhrases,
          forbiddenPhrases: forbiddenPhrases,
          call_error_options: callErrorOptions,
          callErrorOptions: callErrorOptions,
          correct_call_errors: correctCallErrors,
          correctCallErrors: correctCallErrors,

          opening_line: parsed.openingLine,
          openingLine: parsed.openingLine,
          agent_opening: parsed.openingLine,

          first_customer_reply: parsed.firstCustomerReply,
          firstCustomerReply: parsed.firstCustomerReply,
