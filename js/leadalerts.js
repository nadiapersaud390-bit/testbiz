...
function playLeadAlertChime() {
  try {
    ...
    // Soft shimmer overtone on the last note
    const shimmer = ctx.createOscillator();
    const shimGain = ctx.createGain();
    shimmer.type = 'sine';
    shimmer.frequency.setValueAtTime(2093, ctx.currentTime);
    const ts = ctx.currentTime + 0.33;
    shimGain.gain.setValueAtTime(0, ts);
    shimGain.gain.linearRampToValueAtTime(0.3, ts + 0.04);
    shimGain.gain.exponentialRampToValueAtTime(0.001, ts + 0.55);
...
// Lead celebration timing:
// • Alert arrives while this tab is open: celebrate for 30 seconds.
// • Alert arrives while this tab is hidden: celebrate for 5 seconds when the agent returns.
const LEAD_CELEBRATION_VISIBLE_MS = 30000;
const LEAD_CELEBRATION_RETURN_MS  = 5000;
let _leadCelebrationTimer = null;
let _leadAlertActive = false;
let _leadAlertWaitingForReturn = false;
let _leadAlertBlinkMessage = '🔥 New Lead!';
let _leadVisibilityListenersReady = false;
let _leadAlertCardData = null;
let _leadAlertVisibleMs = LEAD_CELEBRATION_VISIBLE_MS;
let _leadAlertReturnMs = LEAD_CELEBRATION_RETURN_MS;

function _ensureLeadCelebrationLayer() {
  let layer = document.getElementById('lead-celebration-layer');
  if (!layer) {
    layer = document.createElement('div');
    layer.id = 'lead-celebration-layer';
...
function _buildLeadConfetti(layer) {
  // Rebuild each time so every lead celebration has a fresh pattern.
  layer.innerHTML = '';
  const fragment = document.createDocumentFragment();
  const shapes = ['', 'round', 'ribbon'];
  const hues = [0, 24, 43, 55, 145, 178, 196, 220, 270, 304, 330];
  const pieceCount = window.innerWidth < 700 ? 85 : 150;

  for (let i = 0; i < pieceCount; i++) {
    const piece = document.createElement('i');
    piece.className = 'lead-confetti-piece ' + shapes[Math.floor(Math.random() * shapes.length)];
    const x = Math.random() * 100;
    const drift = (Math.random() * 34 - 17).toFixed(2);
    const fall = (3.8 + Math.random() * 4.8).toFixed(2);
    const delay = (-Math.random() * 8).toFixed(2);
    const spin = Math.round((Math.random() > .5 ? 1 : -1) * (420 + Math.random() * 980));
    const size = (6 + Math.random() * 11).toFixed(1);
...
function _buildLeadPersonalCard(layer) {
  if (!_leadAlertCardData) return;

  const wrap = document.createElement('div');
  wrap.className = 'lead-personal-card-wrap';
  ...
  title.className = 'lead-personal-title';
  title.textContent = _leadAlertCardData.title || 'Great job!';

  const msg = document.createElement('div');
  msg.className = 'lead-personal-msg';
  msg.textContent = _leadAlertCardData.message || '';

  const quote = document.createElement('div');
  quote.className = 'lead-personal-quote';
  quote.textContent = _leadAlertCardData.quote ? '“' + _leadAlertCardData.quote + '”' : '';
  ...
  if (_leadAlertCardData.quote) card.appendChild(quote);
  if (_leadAlertCardData.stat) card.appendChild(stat);
  wrap.appendChild(card);
  layer.appendChild(wrap);
}

function _stopLeadCelebrationVisuals() {
  if (_leadCelebrationTimer) {
    clearTimeout(_leadCelebrationTimer);
    _leadCelebrationTimer = null;
  }
  const layer = document.getElementById('lead-celebration-layer');
  if (layer) {
    layer.classList.remove('active');
    layer.innerHTML = '';
  }
  document.body.classList.remove('lead-celebration-active');
}

function _showLeadCelebrationFor(durationMs) {
  if (!_leadAlertActive) return;
  _stopLeadCelebrationVisuals();
  const layer = _ensureLeadCelebrationLayer();
  _buildLeadConfetti(layer);
  _buildLeadPersonalCard(layer);
  // Force a paint so repeated alerts restart the entrance cleanly.
  void layer.offsetWidth;
  layer.classList.add('active');
  document.body.classList.add('lead-celebration-active');
  _leadCelebrationTimer = setTimeout(function() {
    dismissLeadAlert();
  }, durationMs);
}

function _handleLeadAlertVisibility() {
  if (!_leadAlertActive) return;

  if (document.hidden) {
    _leadAlertWaitingForReturn = true;
    _stopLeadCelebrationVisuals();
    startTabBlink(_leadAlertBlinkMessage);
    return;
  }

  if (_leadAlertWaitingForReturn) {
    _leadAlertWaitingForReturn = false;
    stopTabBlink();
    _showLeadCelebrationFor(_leadAlertReturnMs || LEAD_CELEBRATION_RETURN_MS);
  }
}

function _prepareLeadVisibilityListeners() {
  if (_leadVisibilityListenersReady) return;
  _leadVisibilityListenersReady = true;
  document.addEventListener('visibilitychange', _handleLeadAlertVisibility);
  window.addEventListener('focus', function() {
    if (!document.hidden) _handleLeadAlertVisibility();
  });
}

function _startLeadAlertExperience(blinkMessage) {
  _prepareLeadVisibilityListeners();
  _leadAlertActive = true;
  _leadAlertBlinkMessage = blinkMessage || '🔥 New Lead!';
  startTabBlink(_leadAlertBlinkMessage);
  ...
  if (document.hidden) {
    ...
    _leadAlertWaitingForReturn = false;
    _showLeadCelebrationFor(_leadAlertVisibleMs || LEAD_CELEBRATION_VISIBLE_MS);
  }
}
...
function _renderAlert({icon, name, msg, quote, firstLead = false, isUploadAlert = false, personal = false, personalCard = null, durationMs = null, returnDurationMs = null, tabMessage = ''}) {
  ...
  const iconEl = document.querySelector('.lab-icon');
  if (iconEl) iconEl.textContent = icon;
  const textEl = document.getElementById('lab-text');
  if (textEl) {
    textEl.innerHTML =
...
function dismissLeadAlert() {
  _leadAlertActive = false;
  _leadAlertWaitingForReturn = false;
  _leadAlertCardData = null;
  _leadAlertVisibleMs = LEAD_CELEBRATION_VISIBLE_MS;
  _leadAlertReturnMs = LEAD_CELEBRATION_RETURN_MS;
  _stopLeadCelebrationVisuals();
  const banner = document.getElementById('lead-alert-banner');
  if (banner) {
...
window.dismissLeadAlert = dismissLeadAlert;
