...
let lookupCache = {};
const LOOKUP_CACHE_DURATION = 60000; // 60 seconds cache

function getCachedLookup(query) {
    const entry = lookupCache[query];
    if (entry && (Date.now() - entry.time) < LOOKUP_CACHE_DURATION) {
        return entry.data;
    }
    return null;
}

function setCachedLookup(query, data) {
    lookupCache[query] = { data: data, time: Date.now() };
}
...
function detectPrank(query) {
    // Check cache first
    const cached = getCachedLookup('prank_' + query);
    ...
    const result = { prankScore: score, prankReasons: reasons, prankVerdict: score >= 80 ? 'definite_prank' : score >= 56 ? 'likely_prank' : score >= 26 ? 'suspicious' : 'clean' };
    
    // Cache the result
    setCachedLookup('prank_' + query, result);
...
function showKnownBadAlert(bad, q) {
    ...
    const c = bad.color, icon = iconMap[bad.type], bg = bgMap[bad.type];
    
    el.innerHTML = '<div style="border-radius:18px;overflow:hidden;border:3px solid ' + c + ';box-shadow:0 0 40px ' + c + '55;animation:fadeSlideIn 0.25s ease-out;">' +
...
function showPrankResult(prank) {
    ...
    el.innerHTML = '<div style="border-radius:18px;overflow:hidden;border:2px solid ' + color + ';box-shadow:0 0 30px ' + color + '44;animation:fadeSlideIn 0.3s ease-out;">' +
        ...
        '<span style="position:relative;z-index:1;font-family:Orbitron,sans-serif;font-weight:900;font-size:0.9rem;color:' + color + ';">' + s + '</span>' +
        '</div>' +
        '<div>' +
        '<div style="font-family:Orbitron,sans-serif;font-size:10px;font-weight:900;text-transform:uppercase;letter-spacing:0.1em;color:' + color + ';margin-bottom:3px;">' + label + '</div>' +
...
function renderLookupHistory() {
    ...
    if (con) {
        con.innerHTML = lookupHistory.map((h, i) => {
            ...
            const badge = s >= 80 ? '<span style="font-size:9px;font-weight:900;background:rgba(239,68,68,0.15);color:#ef4444;border:1px solid rgba(239,68,68,0.3);border-radius:20px;padding:2px 7px;margin-left:6px;">🚨 PRANK</span>' :
                         s >= 56 ? '<span style="font-size:9px;font-weight:900;background:rgba(249,115,22,0.15);color:#f97316;border:1px solid rgba(249,115,22,0.3);border-radius:20px;padding:2px 7px;margin-left:6px;">⚠️ SUSPECT</span>' :
                         s >= 26 ? '<span style="font-size:9px;font-weight:900;background:rgba(234,179,8,0.15);color:#eab308;border:1px solid rgba(234,179,8,0.3);border-radius:20px;padding:2px 7px;margin-left:6px;">👀 CHECK</span>' : '';
            
            return '<div class="search-history-item" onclick="reloadHistory(' + i + ')">' +
                '<div>' +
                '<div class="font-black text-sm text-white">' + escapeHtml(h.query) + badge + '</div>' +
...
function clearHistory() {
    lookupHistory = [];
    try {
        localStorage.setItem('bizlookup_history', '[]');
    } catch (e) {}
    renderLookupHistory();
}
