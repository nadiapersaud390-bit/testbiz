// js/lookup.js - Updated with Google Sheet prank number integration

let KNOWN_PRANK_NUMBERS = [];
...
async function runLookup(){
    ...
    // Check against sheet prank numbers first
    const sheetBad = await checkSheetPrankNumber(q);
    if(sheetBad){
        showKnownBadAlert(sheetBad,q);
        const entry={query:q,prankScore:100,prankVerdict:'definite_prank',prankReasons:['Found in Google Sheet prank number database'],timestamp:new Date().toISOString()};
        lookupHistory.unshift(entry);if(lookupHistory.length>15)lookupHistory=lookupHistory.slice(0,15);
    ...
    if(knownBad){showKnownBadAlert(knownBad,q);const entry={query:q,prankScore:knownBad.type==='prank'?100:knownBad.type==='trucking'?90:70,prankVerdict:knownBad.type==='prank'?'definite_prank':'suspicious',prankReasons:['Found in known bad number database: '+knownBad.label],timestamp:new Date().toISOString()};lookupHistory.unshift(entry);if(lookupHistory.length>15)lookupHistory=lookupHistory.slice(0,15);try{localStorage.setItem('bizlookup_history',JSON.stringify(lookupHistory));}catch(e){}renderLookupHistory();return;}
    const prank=detectPrank(q);showPrankResult(prank);
    ...
    lookupHistory.unshift(entry);if(lookupHistory.length>15)lookupHistory=lookupHistory.slice(0,15);
    try{localStorage.setItem('bizlookup_history',JSON.stringify(lookupHistory));}catch(e){}
    renderLookupHistory();openSite('google');
}
...
async function onLookupInput(val){
    ...
    if(digits.length===10){
        const sheetBad = await checkSheetPrankNumber(q);
        if(sheetBad){showKnownBadAlert(sheetBad,q);return;}
        const bad=checkKnownBadNumber(q);
        if(bad){showKnownBadAlert(bad,q);return;}
    }
    if(!q){const pr=document.getElementById('lookup-prank-result');if(pr){pr.classList.add('hidden');pr.innerHTML='';}}
}
...
// Load prank numbers from Google Sheet on page load
fetchPrankNumbers();
// Refresh every 5 minutes
setInterval(fetchPrankNumbers, 300000);
