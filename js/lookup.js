// lookup.js - Complete working version with Firebase + Google Sheet sync
...
let LIVE_PRANK_NUMBERS=[];
let lastLivePrankFetch=0;
const LIVE_PRANK_CACHE_MS=30000;

try{
    const cached=localStorage.getItem('bizlookup_live_prank');
    if(cached){
        const parsed=JSON.parse(cached);
        if(parsed&&Array.isArray(parsed.numbers)){
            LIVE_PRANK_NUMBERS=parsed.numbers;
            lastLivePrankFetch=Number(parsed.time)||0;
        }
    }
}catch(e){}
...
function addLocalPrankNumber(q){
    const d10=normalizePhone(q).slice(-10);
    if(d10.length<7)return;
    if(!LIVE_PRANK_NUMBERS.includes(d10)){
        LIVE_PRANK_NUMBERS.push(d10);
        try{
            localStorage.setItem('bizlookup_live_prank',JSON.stringify({numbers:LIVE_PRANK_NUMBERS,time:lastLivePrankFetch}));
        }catch(e){}
    }
...
function checkKnownBadNumber(q){
    ...
    if(KNOWN_PRANK_NUMBERS.includes(d10)||LIVE_PRANK_NUMBERS.includes(d10))return{type:'prank',label:'🎭 CONFIRMED PRANK CALLER',color:'#ef4444',msg:'This number is on the confirmed prank caller list. End the call professionally and move on.'};
    if(KNOWN_TRUCKING_NUMBERS.includes(d10))return{type:'trucking',label:'🚛 CONFIRMED TRUCKING — DNQ',color:'#f97316',msg:'This number is a confirmed trucking business. They do not qualify. Do not transfer.'};
    if(KNOWN_NOT_SERIOUS_NUMBERS.includes(d10))return{type:'notserious',label:'⚠️ NOT A SERIOUS LEAD',color:'#eab308',msg:'This number is flagged as not a serious lead. Proceed with extreme caution or skip.'};
    return null;
}
...
function detectPrank(query){
    ...
    if(isPhone){
        ...
        if(/^(\d)\1+$/.test(d)){score+=85;reasons.push('All same digits — clearly fake number');}
        if(['1234567890','0987654321','1234567'].some(s=>d.includes(s.slice(0,7)))){score+=70;reasons.push('Sequential digits — common fake number');}
    ...
    if(/(.)\1{3,}/.test(q)){score+=75;reasons.push('Repeated characters — keyboard mashing');}
    ...
    if(words.length>1&&new Set(words).size===1){score+=75;reasons.push('All words identical — suspicious');}
...
function buildQuery(q){
    if(isPhoneNum(q))return 'For the phone number '+q+', give me: the business name, the name of the owner, the address, and email if possible';
...
function runLookup(){
    ...
    if(knownBad){showKnownBadAlert(knownBad,q);
        const entry={query:q,prankScore:knownBad.type==='prank'?100:knownBad.type==='trucking'?90:70,prankVerdict:knownBad.type==='prank'?'definite_prank':'suspicious',prankReasons:['Found in known bad number database: '+knownBad.label],timestamp:new Date().toISOString()};
...
function openSite(site){
    const q=document.getElementById('lookup-input').value.trim();
    if(!q){document.getElementById('lookup-input').focus();return;}
    const isPhone=isPhoneNum(q),digits=q.replace(/\D/g,''),gq=buildQuery(q);
    const urls={google:'https://www.google.com/search?q='+encodeURIComponent(gq)+'&udm=50',bing:'https://www.bing.com/search?q='+encodeURIComponent(gq)+'&showconv=1',yellowpages:isPhone?'https://www.yellowpages.com/phone-lookup?phone='+digits:'https://www.yellowpages.com/search?search_terms='+encodeURIComponent(q)+'&geo_location_terms=United+States',whitepages:isPhone?'https://www.whitepages.com/phone/'+digits:'https://www.whitepages.com/business/'+encodeURIComponent(q),yelp:'https://www.yelp.com/search?find_desc='+encodeURIComponent(q)+'&find_loc=United+States','411':isPhone?'https://www.411.com/phone/'+digits:'https://www.411.com/business/search?q='+encodeURIComponent(q)};
...
function showPrankResult(prank){
    ...
    const s=prank.prankScore,isD=s>=80,isP=s>=56,label=isD?'🚨 DEFINITE PRANK — END CALL &amp; MOVE ON':isP?'⚠️ LIKELY PRANK — PROCEED WITH CAUTION':'👀 SUSPICIOUS — VERIFY CAREFULLY',color=isD?'#ef4444':isP?'#f97316':'#eab308',bg=isD?'linear-gradient(135deg,rgba(239,68,68,0.22),rgba(185,28,28,0.32))':isP?'linear-gradient(135deg,rgba(249,115,22,0.18),rgba(194,65,12,0.28))':'linear-gradient(135deg,rgba(234,179,8,0.14),rgba(161,98,7,0.24))';
...
// 🔥 FIXED: Complete logPrankCall function with proper Firebase + Sheet sync
async function logPrankCall() {
    ...
    if (!q) {
        ...
        statusEl.innerHTML = '⚠️ Please enter a phone number first';
    ...
    try {
        ...
        if (typeof window.savePrankNumber === 'function') {
            ...
            if (fbResult && fbResult.success) {
                ...
                statusEl.innerHTML = '⚠️ Firebase partial - syncing to sheet...';
            }
        } else {
            console.warn('savePrankNumber function not available');
            statusEl.innerHTML = '⚠️ Firebase not available - saving to sheet only...';
        }
        ...
        try {
            ...
            try {
                ...
                console.warn('Fallback GET also failed:', fallbackErr);
            }
        }
        ...
        if (firebaseSuccess && sheetSuccess) {
            ...
            throw new Error(errorMsg || 'Both Firebase and Sheet sync failed');
        }
        
        setTimeout(() => { statusEl.style.display = 'none'; }, 5000);
        
    } catch (err) {
        console.error('Log prank call error:', err);
        statusEl.style.background = 'rgba(239,68,68,0.12)';
        statusEl.style.border = '1px solid rgba(239,68,68,0.35)';
        statusEl.style.color = '#f87171';
        statusEl.innerHTML = '❌ Failed to log: ' + escapeHtml(err.message || 'Check connection');
        setTimeout(() => { statusEl.style.display = 'none'; }, 5000);
    }
...
function renderLookupHistory(){
    const sec=document.getElementById('lookup-history-section'),con=document.getElementById('lookup-history');
    if(!lookupHistory.length){if(sec)sec.classList.add('hidden');return;}
