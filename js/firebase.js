...
// NEW: Collection for GY/PH performance comparison reports
// Location: RTDB -> agent_stats_reports

window.listenForAgentStatsReports = function(callback) {
    if (!database) {
        console.warn("Firebase not initialized, skipping agent stats reports listener");
        if (callback) callback([]);
        return;
    }
    
    const statsRef = ref(database, 'agent_stats_reports');
    onValue(statsRef, (snapshot) => {
        ...
        // Auto-prune expired reports (older than retention days)
        pruneExpiredStatsReports(data);
    }, (error) => {
        console.error("Agent stats reports listener error:", error);
        if (callback) callback([]);
    });
};

// Save a new agent stats report (GY or PH)
window.saveAgentStatsReport = async function(reportObj) {
    if (!database) return { success: false, error: 'Database not initialized' };
    
    try {
        // Generate a unique ID
        const newRef = push(ref(database, 'agent_stats_reports'));
        ...
        await set(newRef, fullReport);
        console.log(`✅ Saved ${reportObj.region} stats report to Firebase:`, reportId);
        
        // Also update the latest report for this region in a separate reference for quick access
        const latestRef = ref(database, `agent_stats_latest/${reportObj.region}`);
        await set(latestRef, {
            id: reportId,
            reportDate: reportObj.reportDate,
            uploadedAt: reportObj.uploadedAt,
            data: reportObj.data
        });
        
        return { success: true, id: reportId };
    } catch (e) {
        console.error("Error saving agent stats report:", e);
        return { success: false, error: e.message };
    }
};

// Delete a single agent stats report
window.deleteAgentStatsReport = async function(reportId) {
    if (!database) return { success: false, error: 'Database not initialized' };
    
    try {
        // Get the report first to know its region for latest update
        const reportRef = ref(database, `agent_stats_reports/${reportId}`);
        const snapshot = await get(reportRef);
        const report = snapshot.val();
        
        // Delete the report
        await set(reportRef, null);
        console.log(`✅ Deleted agent stats report: ${reportId}`);
        
        // If this was the latest report for its region, find the next latest
        if (report && report.region) {
            const allReportsRef = ref(database, 'agent_stats_reports');
            const allSnapshot = await get(allReportsRef);
            const allData = allSnapshot.val() || {};
            
            const regionReports = Object.keys(allData)
                .filter(key => allData[key].region === report.region)
                .sort((a, b) => new Date(allData[b].uploadedAt) - new Date(allData[a].uploadedAt));
            
            if (regionReports.length > 0) {
                const latestReport = allData[regionReports[0]];
                const latestRef = ref(database, `agent_stats_latest/${report.region}`);
                await set(latestRef, {
                    id: latestReport.id,
                ...
                // No reports left for this region, clear latest
                const latestRef = ref(database, `agent_stats_latest/${report.region}`);
                await set(latestRef, null);
            }
        }
        
        return { success: true };
    } catch (e) {
        console.error("Error deleting agent stats report:", e);
        return { success: false, error: e.message };
    }
};

// Delete ALL agent stats reports
window.deleteAllAgentStatsReports = async function() {
    if (!database) return { success: false, error: 'Database not initialized' };
    
    if (!confirm('⚠️ This will permanently delete ALL GY and PH performance reports. Are you absolutely sure?')) {
    ...
    try {
        ...
        console.log('✅ Deleted all agent stats reports');
        
        // Also clear latest references
        await set(ref(database, 'agent_stats_latest/GY'), null);
        await set(ref(database, 'agent_stats_latest/PH'), null);
        
        return { success: true };
    } catch (e) {
        console.error("Error deleting all agent stats reports:", e);
        return { success: false, error: e.message };
    }
};

// Get the latest report for a specific region (GY or PH)
window.getLatestStatsReport = async function(region) {
    if (!database) return null;
    
    try {
        const latestRef = ref(database, `agent_stats_latest/${region}`);
        const snapshot = await get(latestRef);
        return snapshot.val();
    } catch (e) {
        console.error(`Error getting latest ${region} stats report:`, e);
        return null;
    }
};

// Auto-prune expired reports based on their retentionDays
async function pruneExpiredStatsReports(reportsData) {
    ...
    for (const [id, report] of Object.entries(reportsData)) {
        if (report.expiresAt) {
            ...
            if (expiryDate < now) {
                // Report has expired, delete it
                await set(ref(database, `agent_stats_reports/${id}`), null);
                deletedCount++;
                console.log(`🗑 Pruned expired ${report.region} report from ${report.reportDate}`);
            }
        }
    }
    
    if (deletedCount > 0) {
        console.log(`Pruned ${deletedCount} expired agent stats reports`);
    }
}

// Manual prune function (can be called from admin panel)
window.pruneExpiredStatsReports = async function() {
    if (!database) return;
    ...
    try {
        ...
        const data = snapshot.val();
        if (data) {
            await pruneExpiredStatsReports(data);
        }
        return { success: true };
    } catch (e) {
        console.error("Manual prune error:", e);
        return { success: false, error: e.message };
    }
};
...
function initFirebaseListeners() {
    ...
    // Start periodic prune check for stats reports (every 24 hours)
    setInterval(() => {
        if (typeof window.pruneExpiredStatsReports === 'function') {
            window.pruneExpiredStatsReports();
        }
    }, 24 * 60 * 60 * 1000);
}
